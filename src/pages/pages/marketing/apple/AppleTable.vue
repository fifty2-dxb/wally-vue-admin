...
watch(
  ...
  (merchantGuid) => {
    if (merchantGuid) {
      fetchMarketingData('apple', merchantGuid);
    }
  },
  { immediate: true }
);
  ...
  <div class="d-flex flex-wrap justify-start justify-sm-space-between gap-y-4 gap-x-6 mb-6">
    <div class="d-flex flex-column justify-center">
      <h4 class="text-h4 font-weight-medium">
        {{ $t("Push Notifications for Apple Platform") }}
      </h4>
    ...
    <div class="d-flex gap-4 align-center flex-wrap">
      <VBtn @click="$router.push('/pages/marketing/apple')">{{ $t("Create New Message") }}</VBtn>
    </div>
  </div>
          ...
          <template v-else>
            <VDataTable :headers="headers" :items="marketingStore.marketingData" density="compact" :items-per-page="5">
              <template v-slot:item.platform="{ item }">
                <div v-if="item.platform === 'apple'" class="d-flex align-center">
                  <img class="icon-styling" src="/images/svg/apple-icon-logo.svg" alt="Apple Icon" />
                </div>
                <div v-else>
                  {{ item.platform }}
                </div>
